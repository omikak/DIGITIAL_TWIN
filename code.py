...
# # HEADER
# # -------------------------------------------------------
# st.markdown("<div class='main-header'>🏆 Chandigarh University Thermal Digital Twin</div>", unsafe_allow_html=True)
...
#             (col1, "Avg Temperature", f"{stats.get('avg_temperature', 0)}°C", "Campus Mean"),
#             (col2, "Max Temperature", f"{stats.get('max_temperature', 0)}°C", f"+{stats.get('max_temperature', 0)-stats.get('avg_temperature', 0):.1f}°C"),
...
#                 if not df_all.empty:
#                     fig1 = px.bar(
#                         df_all, x="Zone", y="Temp (°C)", color="Temp (°C)",
...
#     query = st.text_input("🔍 What do you want to know?", placeholder="e.g. Which area is coolest today?")
#     if st.button("Ask"):
...
#     st.markdown("---")
#     st.caption("💡 Tip: Try questions like *What are the hotspots?* or *How to reduce temperature in labs?*")
# 
# # -------------------------------------------------------
# # PAGE 3: ZONE DETAILS
# # -------------------------------------------------------
...
#             for col, name, val in [
#                 (c1, "Temperature", f"{details.get('temperature', 0)}°C"),
...
#                 "🕶️ Avoid direct exposure during peak hours"
#             ]
#             if details.get("is_hotspot"):
#                 recs.insert(0, "🚨 Hotspot Alert! Take shade immediately.")
#             for r in recs:
#                 st.write(r)
#         else:
...
#             fig.update_layout(template="plotly_dark", title="Temperature Forecast", xaxis_title="Date", yaxis_title="°C")
...
# # -------------------------------------------------------
# elif page == "💰 ROI":
#     st.markdown("## 💸 ROI & Cooling Intervention Analysis")
#     interventions = {
...
#         c1, c2, c3, c4 = st.columns(4)
#         for c, title, val in [
#             (c1, "Initial Cost", roi.get("initial_cost", "₹0")),
#             (c2, "Annual Savings", roi.get("annual_savings", "₹0")),
...
#         fig.update_layout(template="plotly_dark", xaxis_title="Year", yaxis_title="Net ₹ (Lakhs)")
#         st.plotly_chart(fig, use_container_width=True)
#     except Exception as e:
#         st.error(f"⚠️ Error: {e}")
# 
...
# # -------------------------------------------------------
# elif page == "🔬 Analytics":
#     st.markdown("## 📊 Smart Insights & Summary")
#     try:
#         data = api_get("/api/v1/insights")
#         col1, col2, col3, col4 = st.columns(4)
#         col1.metric("Temp Variance", f"{data.get('temperature_variance', 0)}°C²")
#         col2.metric("Std Dev", f"{data.get('std_deviation', 0)}°C")
...
# # -------------------------------------------------------
# st.markdown("<hr>", unsafe_allow_html=True)
# st.markdown("<p style='text-align:center; color:#aaa;'>Made with ❤️ for Chandigarh University | Hackathon 2025</p>", unsafe_allow_html=True)
